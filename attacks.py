...
def check_membership(
    model: Model, test_symbol: str, config: runConfig, threshold: int = 100, v: int = 0
) -> bool:
    ...
    if v > 0:
        ...
        print(
            f"Symbol: '{test_symbol}', Loss: {loss}, Confidence Score: {confidence_score}, Prediction: {loss < threshold}"
        )
...
def perform_attack(config: runConfig, weights_path: str = "LSTM.pth") -> None:
    ...
    all_symbols = read_file("data/all_symbols.txt")
...
def train_shadow_models(
    config: runConfig, all_symbols: list[str], num_shadow_models: int = 3
) -> list[Tuple[Model, Dataset]]:
    shadow_models = []
...
def prepare_attack_data(
    shadow_models: list[Tuple[Model, Dataset]]
...
def train_attack_model(
    attack_features: np.ndarray, attack_labels: np.ndarray
